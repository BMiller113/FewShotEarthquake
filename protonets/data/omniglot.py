...
import protonets
from protonets.data.base import (
    convert_dict,
    CudaTransform,
...
def convert_tensor(key, d):
    d[key] = (
        1.0
        - torch.from_numpy(np.asarray(d[key], np.float32))
        .transpose(0, 1)
        .contiguous()
...
def load_class_images(d):
    if d["class"] not in OMNIGLOT_CACHE:
        ...
        loader = torch.utils.data.DataLoader(
            image_ds, batch_size=len(image_ds), shuffle=False
        )
...
def load(opt, splits):
    ...
    for split in splits:
        ...
        ret[split] = torch.utils.data.DataLoader(
